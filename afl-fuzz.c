...
llvm_mode = 0;				// Use LLVM mode to generate mutate data
 ...
 *****************/
	if (llvm_mode) {
		...
		stage_short = custom_stage_short;
	}
...
int main(int argc, char** argv) {
	...
	while ((opt = getopt(argc, argv, "+i:o:f:m:t:I:T:sdYnCB:S:M:x:QD:b:l:L:pPc:w:A:eV")) > 0)
		...
		case 'L':
			llvm_mode = 1;
			custom_stage_name = optarg;
			custom_stage_short = "custom";
			break;
	...
	while (1) {
		...
		if (!llvm_mode) {
			...
			skipped_fuzz = fuzz_one_llvm(use_argv);
		}
